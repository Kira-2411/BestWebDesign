...
function buildReason(profile, university, major, delta, trend) {
  ...
  return `Điểm của bạn ${diffText}, ${interestNote}, trường ở ${university.city}, ${regionLabelText}. Xu hướng điểm chuẩn 3 năm qua: ${trend.toLowerCase()}.`;
}
...
export function computeMatchesFromPairs(groupedPairs, profile) {
  ...
  Object.values(groupedPairs).forEach((pair) => {
    ...
    matchScore += interests.includes(major.category) ? 22 : 0;

    if (region && region !== 'all') {
      matchScore += matchesRegion(university, region) ? 12 : -4;
    } else {
      matchScore += 8;
    }
    ...
    if (maxTuition > 0) {
      if (university.tuition <= maxTuition) {
        ...
        matchScore += Math.max(-12, 4 - over * 0.8);
      }
    } else {
      matchScore += 6;
    }
    ...
    matches.push({
      ...
      trend,
      score: matchScore,
      reason: buildReason(profile, university, major, delta, trend),
    });
  });
